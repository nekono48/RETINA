...
pub fn ExtAudioFileGetProperty(
    ...
    match in_property_id {
        x if x == kExtAudioFilePropertyFileDataFormat || x == kExtAudioFilePropertyClientDataFormat => {
        ...
        kExtAudioFilePropertyFileLengthFrames => {
            ...
            if !out_property_data.is_null() {
                ...
                let frames = host_object.audio_file.packet_count() as i64 * desc.frames_per_packet as i64;
...
pub fn AudioFileOpenURL(
    ...
    if in_file_type_hint != 0 && in_file_type_hint != kAudioFileCAFType {
        unimplemented!("Unsupported file type hint");
    }
    ...
    State::get(&mut env.framework_state).audio_files.insert(guest_af, AudioFileHostObject { audio_file });
...
pub fn AudioFileOpenWithCallbacks(
    ...
    let status: OSStatus = read_callback.call_from_host(env, (client_data, 0_i64, size_u32, data_ptr, bytes_read_ptr));
...
fn property_size(property_id: AudioFilePropertyID) -> Option<GuestUSize> {
    match property_id {
        ...
        kAudioFilePropertyAudioDataByteCount | kAudioFilePropertyAudioDataPacketCount => Some(guest_size_of::<u64>()),
...
pub fn AudioFileGetPropertyInfo(
    ...
    if let Some(size) = property_size(in_property_id) {
        if !out_data_size.is_null() { env.mem.write(out_data_size, size as u32); }
...
pub fn AudioFileGetProperty(
    ...
    return_if_null!(in_audio_file);
    let Some(req_size) = property_size(in_property_id) else { return kAudioFileUnsupportedProperty; };
    if env.mem.read(io_data_size) != req_size as u32 { return kAudioFileBadPropertySizeError; }

    let host_object = State::get(&mut env.framework_state).audio_files.get_mut(&in_audio_file).unwrap();

    match in_property_id {
        kAudioFilePropertyDataFormat => { env.mem.write(out_property_data.cast(), get_asbd(host_object)); }
        ...
        kAudioFilePropertyEstimatedDuration => {
            let desc = host_object.audio_file.audio_description();
            let dur = host_object.audio_file.byte_count() as f64 * desc.frames_per_packet as f64 / (desc.bytes_per_packet as f64 * desc.sample_rate);
...
pub fn AudioFileReadBytes(
    ...
    return_if_null!(in_audio_file);
    let host_obj = State::get(&mut env.framework_state).audio_files.get_mut(&in_audio_file).unwrap();
    let to_read = env.mem.read(io_num_bytes);
    let buf = env.mem.bytes_at_mut(out_buffer.cast(), to_read);

    let read = host_obj.audio_file.read_bytes(in_start.try_into().unwrap(), buf).unwrap();
...
pub fn AudioFileReadPackets(
    ...
    let to_read = env.mem.read(io_num_packets);
    let start_byte = in_start_packet * pkt_size as i64;

    env.mem.write(out_num_bytes, to_read * pkt_size);
    AudioFileReadBytes(env, in_audio_file, use_cache, start_byte, out_num_bytes, out_buffer)
}

pub fn AudioFileReadPacketData(
    env: &mut Environment, in_af: AudioFileID, cache: bool, out_bytes: MutPtr<u32>,
    descs: MutVoidPtr, in_start_pkt: i64, io_pkts: MutPtr<u32>, out_buf: MutVoidPtr,
) -> OSStatus {
